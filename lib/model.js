  ...
  for (var k = 0; k < size; k++) {
    ...
    if (properties[p]) {
      ...
      if (relationType === 'belongsTo' && propVal != null) {
        ...
        // if populated data is from Atlas we need to pick from the key _id
        if (ctor.relations[p].keyTo && ctor.relations[p].keyTo === 'id') {
          self.__data[ctor.relations[p].keyFrom] = propVal[ctor.relations[p].keyTo] || propVal['_id'];
        }
